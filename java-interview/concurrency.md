...
__volatile__ -- ключевое слово volatile используется для пометки переменной, как хранящейся только 
  ...
  В Java ключевое слово "volatile" используется для обозначения переменных, которые могут быть изменены несколькими потоками. 
Основное назначение "volatile" состоит в том, чтобы гарантировать видимость изменений переменной другим потокам.
Когда переменная объявлена как "volatile", это означает, что ее значение может быть изменено другими потоками, и каждый 
  поток всегда видит самое последнее изменение этой переменной. Без использования "volatile" или других синхронизационных
 механизмов (например, блокировки), потоки могут кэшировать значения переменных и не обновлять их, что может привести к 
 непредсказуемому поведению программы.

Вот несколько основных ситуаций, когда полезно использовать ключевое слово "volatile" в Java:
- Флаги состояния: Если переменная используется для управления состоянием программы и ее значение может измениться из 
одного потока, а считывается из других, использование "volatile" гарантирует, что изменения будут видны всем потокам.

- Двойная проверка блокировки: Используется в шаблоне двойной проверки блокировки для инициализации ленивых объектов без синхронизации.

- Счетчики событий: Если переменная используется для подсчета событий в нескольких потоках, "volatile" гарантирует, что 
каждый поток видит актуальное значение счетчика.
...
Шаг 1: Создание класса, реализующего Runnable
...
public class Main {
    public static void main(String[] args) {
        MyRunnable myRunnable = new MyRunnable(); // Создание экземпляра Runnable
        Thread thread = new Thread(myRunnable); // Создание потока, передача Runnable в конструктор
...
public class Main {
    public static void main(String[] args) {
        MyThread myThread = new MyThread(); // Создание экземпляра потока
        myThread.start(); // Запуск потока
    }
}
```

Анонимный класс Thread
```java
public class Main {
    public static void main(String[] args) {
...
Использование лямбда-выражения (начиная с Java 8)
С Java 8 можно использовать лямбда-выражения для реализации интерфейса Runnable.
```java
public class Main {
    public static void main(String[] args) {
        Runnable runnable = () -> {
        ...
        Thread thread = new Thread(runnable); // Создание потока с использованием лямбда-выражения
        thread.start(); // Запуск потока
    }
}
```
...
```java
void join();        
void join(long millis); // поток в котором вызвали join будет ждать либо пока поток не закончит работу, либо long
