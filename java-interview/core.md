...
+ [Какие преимущества у 17 java перед 11?](core.md#Какие-преимущества-у-17-java-перед-11)
...
``Обертка`` — это специальный класс, который хранит внутри себя значение примитива. Классы-обертки позволяют нивелировать
недостатки, которые есть у примитивных типов. Самый очевидный из них — ``примитивы не имеют методов.`` Например, у них 
нет метода toString(), поэтому ты не сможешь, например, преобразовать число int в строку. Допустим, у нас есть строка,
про которую мы точно знаем, что она содержит число. Тем не менее, в случае с примитивным типом int мы никак не сможем 
это число из строки достать и превратить, собственно, в число. Существуют такие статические методы как ``Integer.MAX_VALUE``
...
Примитивные типы потому и называют примитивными, потому что они лишены многих “тяжеловесных” особенностей объектов. 
Примитивы нужны когда есть простые операции и важна скорость. 
...
Исключения делятся на несколько классов, но все они имеют общего предка — класс `Throwable`, потомками
которого являются классы `Exception` и `Error`.__Ошибки (Errors)__ представляют собой более серьёзные 
проблемы, которые, согласно спецификации Java, не следует обрабатывать в собственной программе, 
поскольку они связаны с проблемами уровня JVM. Например, исключения такого рода возникают, если 
закончилась память доступная виртуальной машине.__Исключения (Exceptions)__ являются результатом 
проблем в программе, которые в принципе решаемы, предсказуемы и последствия которых возможно устранить 
внутри программы. Например, произошло деление целого числа на ноль.

![pick](resources/exception.png)
...
Type Safe (безопасность, будут ошибки на уровне компиляции) &  Reusable Code Генерики позволяют создавать 
...
public class ParameterizedClass {
    ...
    public static void main(String[] args) {
        Info <String> value1 = new Info<>("DEMON");
        System.out.println(value1);

        Info<Integer> value2 = new Info<>(111);
...
```java
// параметризированный метод возвращает второй элемент любого ArrayList
public <T> T getSecondElement(ArrayList<T> a1) {
    return a1.get(1);
        }
```
...
## Какие преимущества у 17 java перед 11?

ChatGPT
Переход с Java 11 на Java 17 включает несколько значимых изменений и улучшений. Вот несколько преимуществ Java 17 по сравнению с Java 11:

- Улучшения языка и среды выполнения: Java 17 включает в себя улучшения языка, например, в Java 17 появилась поддержка 
sealed classes (запечатанные классы), которая облегчает контроль над наследованием и полиморфизмом. Это улучшение 
позволяет разработчикам создавать более безопасные и надежные приложения.

- Производительность: В каждом новом выпуске Java вносятся улучшения производительности. Например, с Java 17 можно ожидать 
оптимизаций в среде выполнения и улучшений сборки мусора, что может привести к улучшению производительности в ваших приложениях.

- Новые библиотеки и API: Java 17 может включать новые библиотеки и API, которые расширяют функциональные возможности языка. 
Это может включать в себя улучшенные библиотеки для работы с потоками, файлами, сетевыми операциями и т.д.

- Обновления безопасности: Каждая новая версия Java включает в себя обновления безопасности, что обеспечивает большую защиту от известных уязвимостей.

- Долгосрочная поддержка (LTS): Важно отметить, что Java 11 является долгосрочно поддерживаемой версией (LTS), тогда как 
Java 17 на момент выхода может быть не LTS. Для некоторых организаций и проектов это может быть важным фактором при выборе версии Java.

- Инструменты и экосистема: С каждым выпуском Java улучшаются инструменты разработки и поддержки (IDE, фреймворки и т.д.),
что может сделать разработку более удобной и эффективной.

В итоге, переход с Java 11 на Java 17 может быть полезным для получения доступа к новым функциональным возможностям языка, 
улучшения производительности и общей безопасности приложений. Однако, перед обновлением важно учитывать особенности и 
требования вашего проекта, включая совместимость с библиотеками и другими зависимостями.
